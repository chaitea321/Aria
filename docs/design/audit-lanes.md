...
| **backend** | `backend/**` (esp. `app.py`), `Aria---Music-Browser-Info.plist` (ATS), `Services/TLSPinningDelegate.swift`, `.github/workflows/**` | Aria | Backend — Cache & Reliability · Security & Abuse · Streaming & Latency | `feat/backend-cover-art` | in-PR |
