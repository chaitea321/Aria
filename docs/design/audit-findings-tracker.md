...
| ⬜ open | medium | M | Wire local tracks to the /api/cover backend so tracks with no embedded art get a web cover | Backend `/api/cover` shipped (#20) but nothing calls it. When embedded extraction (extractArtwork) returns nil, LocalLibraryManager should fetch `/api/cover?title=&artist=&duration=`, download the image into the artwork/ dir (relative-path storage from #13), and set artworkFileName; also backfill existing coverless tracks in the healMissingArtwork pass. Opportunistic (needs network; offline → stays placeholder), best-effort, one fetch per track. |
