...
| ✅ done | medium | M | Local/offline tracks show no artwork — embedded cover art is not reliably extracted | Fixed on feat/data-local-artwork: LocalLibraryManager.extractArtwork (LocalLibraryManager.swift) now delegates to a new `loadArtworkData(from:)` that tries, in order, (1) the common-identifier artwork item filtered out of the asset's full `.metadata` set plus a broader identifier/key heuristic over that same set, (2) every format-specific metadata set the asset advertises via `availableMetadataFormats`/`loadMetadata(for:)` (covers ID3 `APIC` for MP3, iTunes `covr` for MP4/M4A, QuickTime/ISO user data), and (3) the original `.commonMetadata`/`commonKey == "artwork"` scan as a final fallback — each step also coerces non-`dataValue` payloads via `.value`. Extraction is generic over a new `MetadataLoading` protocol (`AVURLAsset` conforms via an adapter extension) so the selection logic is unit-testable without real audio fixtures. Extension sniffing extended with GIF magic bytes alongside JPEG/PNG. Display path (LocalTrack.artworkURL → thumbnailURL → AsyncCachedImage) was already correct and untouched. Coverage: 9 new XCTest cases in Tests/LocalLibraryManagerTests.swift exercising extension sniffing and artwork-item selection (common identifier, ID3 APIC by identifier and by raw key, iTunes covr, legacy common-metadata fallback, empty-data-treated-as-absent, no-artwork-returns-nil) using synthetic `AVMutableMetadataItem`s — no binary audio fixtures added. Full AriaTests suite: 222 tests, 0 failures (2 skipped live-integration tests unrelated to this change). |
